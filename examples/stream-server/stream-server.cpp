// Real-time speech recognition of input from a network socket
//
// Based on the streaming example in stream.cpp
//
#include "common.h"
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr, "%s: processing '%s' (%d samples, %.1f sec), %d threads, %d processors, %d beams + best of %d, lang = %s, task = %s, %stimestamps = %d ...\n",
                    __func__, fname_inp.c_str(), int(pcmf32_full.size()), float(pcmf32_full.size())/WHISPER_SAMPLE_RATE,
        ...
        // Samples processed in the previous iteration
